...
class WishlistHeader extends StatelessWidget {
  ...
  const WishlistHeader({super.key, required this.savedItemsCount, required this.onSelectPressed, required this.onBackPressed});
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(s.myWishlist, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: AppColors.userNameColor)),
                const SizedBox(height: 2),
                Text(s.savedItems(savedItemsCount), style: const TextStyle(fontSize: 14, color: AppColors.greysoft)),
              ],
            ),
          ),
